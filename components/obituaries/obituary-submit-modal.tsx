...
import { ScrollText, X, CheckCircle2, UploadCloud, ChevronLeft, AlertTriangle, Wand2 } from 'lucide-react'
...
export function ObituarySubmitModal({ onClose }: { onClose: () => void }) {
  ...
  const handlePhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Replace preview with bg-removed version
      URL.revokeObjectURL(originalUrl)
      setPhotoPreview(url)
      // Replace the file blob so we upload the bg-removed PNG
      ...
      // Fall back to original — no error shown to user
    } finally {
      setBgRemoving(false)
    }
  }
          ...
          {done ? (
            ...
            <div className="px-6 py-5 space-y-5 max-h-[80vh] overflow-y-auto">
                  ...
                  {bgRemoving ? (
                    ...
                    <>
                      ...
                      <p className="text-[10px] text-muted-foreground mt-0.5">
                        {bgRemoved ? '✓ Background removed automatically' : 'PNG, JPG · max 5 MB · background auto-removed'}
                      </p>
                    </>
                  )}
                </div>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <Button type="button" onClick={handleNext} disabled={bgRemoving} className="flex-1 h-11 font-bold rounded-xl">
                  {bgRemoving ? 'Processing photo…' : 'Review →'}
                </Button>
              </div>
            </div>
