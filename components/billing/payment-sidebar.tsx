...
export function PaymentSidebar({ paymentInfo, method }: {
  paymentInfo: PaymentInfo | null
  method?: string
}) {
  const supabase = createClient()

  // Determine what to show based on selected payment method.
  // method === undefined means we're on the review step — show both.
  const showGcash = !method || method === 'gcash'
  const showBank  = !method || method === 'bdo_bank'
  // Cash (Counter) → nothing payment-specific to show
  ...
  return (
    <div className="space-y-4 lg:sticky lg:top-24">
      ...
      {/* Cash (Counter) — just a helpful note */}
      {method === 'cash' && (
        <div className="bg-muted/30 border border-border/60 rounded-2xl px-5 py-4 text-xs text-muted-foreground leading-relaxed space-y-1">
          <p className="font-bold text-foreground text-xs">Cash Payment</p>
          <p>Please proceed to our counter to complete your payment in person.</p>
        </div>
      )}
