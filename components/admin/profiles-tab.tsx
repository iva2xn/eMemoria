...
import { Trash2, X, UserCog, RotateCcw, Clock } from 'lucide-react'
...
export function ProfilesTab({ currentRole, highlightDeletedEmail }: { currentRole: UserRole; highlightDeletedEmail?: string | null }) {
  ...
  // ── Delete account (soft-delete via migration 029) ───────
  ...
  const handleRecover = async (account: DeletedAccount) => {
    ...
    // Send recovery/re-invite email via Supabase Auth admin invite
    // This sends the user a magic link to set a new password
    const { error: inviteErr } = await supabase.auth.admin
      ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await (supabase.auth as any).admin.inviteUserByEmail(account.email, {
          data: { name: account.name },
        })
      : { error: new Error('Admin auth not available on client') }

    // Whether or not invite succeeds, send a notification email via our API
    ...
    if (inviteErr) console.warn('[recover] invite error (non-fatal):', inviteErr)
  }
